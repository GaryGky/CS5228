...
def plot_kmeans_clustering(X, labels, cluster_centers):
    ...
    for cluster_id in np.unique(labels):
        ...
        if X_cluster.shape[0] > 0:
            ...
            for x in X_cluster:
                plt.plot([x[0], cluster_centers[cluster_id][0]], [x[1], cluster_centers[cluster_id][1]], '--',
                         linewidth=0.5, color='k'.format(cluster_id))
...
def inter_itemsets(a, b):
    itemset = set(a).intersection(b)
    itemset = tuple(sorted(itemset))
    return itemset
...
def show_top_rules(transactions, min_support=0.0, min_confidence=0.0, k=5, sort='lift', reverse=True, id_map=None,
                   rhs=None):
